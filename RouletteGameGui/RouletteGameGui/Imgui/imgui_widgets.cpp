...
void ImGui::TextV(const char* fmt, va_list args)
{
    ...
    // Calculate alpha based on time or frame count
    static float alpha = 0.0f; // Starting alpha
    float fade_duration = 3.0f; // Duration for full fade-in (adjust as needed)
    alpha = ImGui::GetTime() / fade_duration; // Example: time-based alpha

    // Clamp alpha to [0, 1]
    alpha = ImClamp(alpha, 0.0f, 1.0f);

    // Use PushStyleColor to set text color with alpha
